---
title: |
  | \LARGE \bf{Analyzing US Wildfires}
  |
  | by
  |
...
## b. Data Description
The cleaned data set consist of 188,017 observations on the following 16 variables. 3 new variables have been added through data wrangling. Belo are the description of all the variables. Apart from this data set, `states` data set from R was used to perform data transformation.
\
\
...
**Potential Use Case:** 
* From the Data set, we know that the following fire sizes A to G represents the respective acres of land burnt, as mentioned below
    + A= greater than 0 acres but less than or equal to 0.25 acres 
    + B=0.26-9.9 acres
    + C=10.0-99.9 acres
    + D=100-299 acres
    + E=300 to 999 acres
    ...
    + G=5000+ acres
- From the below plot, it is interesting to note that some states such as Alaska, Orgeon, Washington have taken more days to resolve the wildfire although less area was burnt. 
- Alarmingly, states like "Alaska" seems to have struggle resolving wildfires of any size. This should be investigated further.
...
**For Research Question 4 - Multiple Linear Regression model**

- Here we test the data, asis for linearity. Here we use log transformation on `fire_size` as the response variable and feature such as `disc_month, season, latitude, longitude, cause of fire, fire_year, owner_descr` as the predictor variables. 
- Here, we see that the assumptions of Multiple Linear Regression are not satisfied. Specifically, the Normal Q-Q plot doesn't appear to be linear. The multiple R -squared is 12.77% indicating there's not a very statistically significant relationship between the response and predictors. The Cook's distance plot also shows some highly influential point(153849).
\

```{r, echo=FALSE, fig.align='center'}
#analysis of residuals
model0 <- lm(data = wildfires_clean_with_states, log10(fire_size)~disc_month+season+latitude+longitude+stat_cause_descr+fire_year+owner_descr)
...
- As per this study, smaller southern states like Georgia, Florida and Alaska must be more prepared with the right equipment and safety measures to fight wildfires.
- The MLR model of this study shows that a 12% of variation in the response variable can be explained by the predictors we considered.
...
- Our MLR model doesn't show a statistically significant relationship, this model can be further improved by performing backward AIC to remove the least contributing predictors and retain the predictor variables that are statistically significant. We would also need to remove highly influential point(Cook's Diagnostic plot) as it lies above the 0.05 level distance.
